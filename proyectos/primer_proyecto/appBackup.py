...
def extract_document_fields(lines: List[str]) -> dict:
    ...
    stop_labels = ["NOMBRE", "NOMBRES", "DOMICILIO", "DIRECCION", "DIRECCIÓN", "ADDRESS"]
    nombre = find_label_value(["NOMBRE", "NAME", "NOMBRES"], stop_labels)
...
class DocumentOcrService:
    ...
    def predict(self, image: Image.Image) -> DocumentResponse:
        ...
        lines = [normalize_line(text) for _, text, _ in results if text.strip()]
        ...
        if label_boxes:
            ...
            roi_results = self.reader.readtext(roi, paragraph=False)
            roi_lines = [normalize_line(text) for _, text, _ in roi_results if text.strip()]
            if roi_lines and len(roi_lines) >= len(lines):
                lines = roi_lines
        ...
        if not fields.get("nombre") and not fields.get("domicilio"):
            ...
            fallback_results = self.reader.readtext(prepared, paragraph=False)
            fallback_lines = [
                normalize_line(text) for _, text, _ in fallback_results if text.strip()
            ]
            if fallback_lines:
